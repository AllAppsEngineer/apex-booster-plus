...
Widget _buildMediaSlot(String path, Color accent, AppStrings s) {
  final isVideo = _isVideoPath(path);
  return Expanded(
    child: ClipRRect(
      borderRadius: BorderRadius.circular(8),
      child: isVideo
          ? _videoPlaceholder(accent, s)
          : Image.file(
              File(path),
              fit: BoxFit.cover,
              width: double.infinity,
              errorBuilder: (_, __, ___) => _videoPlaceholder(accent, s),
            ),
    ),
  );
}

Widget _videoPlaceholder(Color accent, AppStrings s) {
  return Container(
    color: const Color(0xFF1A1A1A),
    child: Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        Icon(Icons.videocam_rounded, color: accent, size: 28),
